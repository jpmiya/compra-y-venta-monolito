...
- **CIERRE de la migración ✅ — gateway, stack completo validado, CI, docs:**
  - **API Gateway (nginx :8080)** en [gateway/nginx.conf](gateway/nginx.conf) + servicio `gateway` en compose. Única puerta de entrada: enruta `/personas|/usuarios|/direcciones→Identidad`, `/billetera→Billetera`, `/productos|/busqueda→Catálogo`, `/deliveries→Delivery`, `/carrito→Carrito`, y **devuelve 403 a todo `/interno/*`** (eso queda solo entre servicios). Decisión: nginx sobre un gateway FastAPI propio o Kong/Traefik — cero código, estándar, alcanza para el TP.
  - **Stack completo levantado y validado**: `docker compose --profile microservices up -d --build` → 14 containers healthy (5 servicios + 5 DBs + gateway + rabbit + monolito + su DB). Verificado en vivo: routing del gateway (`/productos` 200, `/carrito` sin token 403, `/interno/*` 403), REST inter-servicio real (Identidad batch, Billetera debitar), y **cadena async real**: comando publicado → consumer del container Delivery → fila en `delivery_db` → `DeliveriesCreado` de vuelta por el canal de la saga ✅.
  - **Lo ÚNICO que falta probar a mano** (necesita las credenciales que tiene Juampi): el flujo autenticado end-to-end con token Firebase real — poner `firebase-service-account.json` en la raíz, `FIREBASE_WEB_API_KEY` en `.env`, levantar el stack y correr la colección Postman nueva ([docs/postman_microservicios.json](docs/postman_microservicios.json), ya apunta al gateway y guarda el token sola). En esta máquina quedó un placeholder de `firebase-service-account.json` sin credenciales (gitignoreado) solo para que el mount de Docker funcione.
  - **CI por servicio**: `.github/workflows/ci.yml` ahora tiene el job `test-services` (matrix: identidad/billetera/catalogo/delivery/carrito, cada uno con su Postgres en el puerto que espera su conftest). El job del monolito sigue igual.
  - **Diagramas nuevos** en `docs/diagramas/`: [c4_contenedores_microservicios.md](docs/diagramas/c4_contenedores_microservicios.md) (los 5 servicios + gateway + broker + database-per-service) y [uml_secuencia_checkout_saga.md](docs/diagramas/uml_secuencia_checkout_saga.md) (camino feliz, compensación y retry desde el log). README de diagramas actualizado.
  - **README** reescrito arriba: las dos arquitecturas conviven, cómo levantar microservicios, tests y CI.
  - **Decisión sobre el monolito**: queda intacto (63/63) como base de comparación del TP; la UI web (Jinja2) y `notificaciones` no se migran (fuera de alcance, plan §2). No se lo convirtió en fachada: la cara al cliente de los microservicios es el gateway.
...
1. **Único paso manual pendiente**: checkout end-to-end con auth real. Con tus credenciales: `firebase-service-account.json` en la raíz + `FIREBASE_WEB_API_KEY` en `.env` → `docker compose --profile microservices up -d --build` → importar `docs/postman_microservicios.json` → correr `00-Auth` y después el flujo completo (crear persona/usuario/dirección → producto → cargar billetera → carrito → checkout → `GET /carrito/checkout/{saga_id}` hasta ver `delivery_estado: confirmado` → tomar/entregar el delivery).
2. Revisar el push de CI: primer run del job `test-services` (matrix de 5) — debería salir verde directo.
3. Opcional según enunciado: probar en vivo el camino 402 (cargar poco saldo y ver la compensación) y el retry (frenar el container de rabbit antes del checkout y verlo reenviarse al volver).
