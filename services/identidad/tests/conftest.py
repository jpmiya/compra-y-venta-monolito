...
os.environ.setdefault(
    "DATABASE_URL",
    "postgresql+asyncpg://postgres:pass123@localhost:5435/identidad_test",
)
...
from app.adapters.persistence.models import Persona, Usuario  # noqa: F401 — needed for Base.metadata

TEST_DATABASE_URL = os.getenv(
    "TEST_DATABASE_URL",
    "postgresql+asyncpg://postgres:pass123@localhost:5435/identidad_test",
)
TEST_FIREBASE_UID = "test-firebase-uid-identidad-001"
...
async def limpiar_tablas():
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE TABLE usuario_roles, usuarios, direcciones, personas, roles CASCADE"
                )
            )
    finally:
        await engine.dispose()
...
async def internal_client() -> AsyncClient:
    """Cliente sin auth para endpoints /interno/."""
    request_engine = create_async_engine(TEST_DATABASE_URL)
    request_session_factory = async_sessionmaker(request_engine, expire_on_commit=False)
