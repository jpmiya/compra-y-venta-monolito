...
async def actualizar_persona(db: AsyncSession, persona: Persona, data: PersonaUpdate) -> Persona:
...
async def get_usuario_by_firebase_uid(db: AsyncSession, firebase_uid: str) -> Optional[Usuario]:
    result = await db.execute(
...
async def listar_usuarios_de_persona(db: AsyncSession, persona_id: uuid.UUID) -> List[Usuario]:
    result = await db.execute(
...
async def crear_usuario(db: AsyncSession, persona_id: uuid.UUID, data: UsuarioCreate) -> Usuario:
...
async def get_direccion_by_id(db: AsyncSession, direccion_id: uuid.UUID) -> Optional[Direccion]:
